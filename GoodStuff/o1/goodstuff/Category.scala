...
class Category(val name: String, val unit: String) {
  ...
  // This implementation of favorite/addExperience will be redone multiple times in 
  // different ways during the course as we encounter additional programming concepts.
  
  
  /** Adds a new experience to this category. */
  def addExperience(newExperience: Experience) = {
    this.experiences += newExperience
    val newFave = if (this.fave.isEmpty) newExperience else newExperience.chooseBetter(this.fave.get)
