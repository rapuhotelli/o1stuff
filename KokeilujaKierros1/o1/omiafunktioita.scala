...
object omiafunktioita {
  ...
  def saesta(ulinaa: String, nuotit: String) = {
    println(ulinaa)
    o1.soita(nuotit)
  }
  
  def kurssiarvosana(tehtavat: Int, tenttiBonukset: Int, aktiivisuus: Int) = {
    val pisteet = tehtavat + tenttiBonukset + aktiivisuus
    min(pisteet, 5)  
  }

  def pisteet(voitot: Int, tasapelit: Int) = voitot*3 + tasapelit
  
  def joukkueenTiedot(jNimi: String, voitot: Int, tasapelit: Int, tappiot: Int) = {    
    val pelit = voitot + tasapelit + tappiot
    val voitotStr = voitot+"/"+pelit+" voittoa, "
    val tasapelitStr = tasapelit+"/"+pelit+" tasapeliä, "
    val tappiotStr = tappiot+"/"+pelit+" tappiota, "
    jNimi+": "+voitotStr+tasapelitStr+tappiotStr+pisteet(voitot, tasapelit)+" pistettä"
  }
  ...
  def tuplaaPisteet(pisteet: Buffer[Int], pelaajaId: Int) = {
    pisteet(pelaajaId-1) = pisteet(pelaajaId-1)*2
  }
  def sakko(pisteet: Buffer[Int], pelaajaId: Int, sakotus: Int) = {
    val alkuperaisetPisteet = pisteet(pelaajaId-1)
    val uudetPisteet = max((pisteet(pelaajaId-1)-sakotus), 1)
    pisteet(pelaajaId-1) = uudetPisteet
    val erotus = alkuperaisetPisteet-uudetPisteet
    erotus
  }
